using System.Collections;
using System.Collections.Generic;
using System.Data.SqlTypes;
using UnityEngine;

public class MyMonsterShaderTest : MonoBehaviour
{
    #region [成员变量]
    public GameObject Monster;
    private Material mat;
    #endregion
    public Color drugcolor;
    #region [Start/Update]
    // Start is called before the first frame update
    void Start()
    {
        mat = Monster.GetComponentInChildren<SkinnedMeshRenderer>().sharedMaterial;
    }

    // Update is called once per frame
    void Update()
    {
        
    }
    #endregion

    #region  [GUI]
    void OnGUI()
    {
        if (GUI.Button(new Rect(10,10,150,50),"被击"))
        {
            StopAllCoroutines();//打断所以协程
            StartCoroutine(WaitBehit());
        }

        if (GUI.Button(new Rect(10, 70, 150, 50), "中毒"))
        {
            StopAllCoroutines();
            StartCoroutine(WaitDrug(2f));
        }

        if (GUI.Button(new Rect(10, 130, 150, 50), "消散"))
        {
            StopAllCoroutines();
            StartCoroutine(WaitFade(1f));
        }
        
    }
    #endregion

    #region ["被击"]
    IEnumerator WaitBehit()
    {
        mat.SetColor("_Color",Color.white);

        yield return new WaitForSeconds(0.1f);

        mat.SetColor("_Color",Color.black);
            
    }
    #endregion

    #region ["中毒"]
    IEnumerator WaitDrug(float time)
    {
        float _time = 0;
        while (true)
        {
            _time += Time.deltaTime;
            Debug.Log(_time);
            yield return new WaitForEndOfFrame();

            if (_time > time)
            {
                yield break;
            }
            else
            {
                Color _color = Color.Lerp(drugcolor, Color.black, _time / time);
                mat.SetColor("_Color",_color);
            }
        }
    }
    #endregion

    #region ["消散"]
    IEnumerator WaitFade(float time)
    {
        float _time = 0;
        while (true)
        {
            _time += Time.deltaTime;
            yield return new WaitForEndOfFrame();

            if (_time > time)
            {
                mat.DisableKeyword("_DISSOLVEENABLED_ON");
                mat.SetFloat("_Clip", 0);
                yield break;
            }
            else
            {
                mat.EnableKeyword("_DISSOLVEENABLED_ON");
                mat.SetFloat("_Clip", _time / time);
            }
        }
    }
    #endregion

}
